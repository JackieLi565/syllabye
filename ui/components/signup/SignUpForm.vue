...
const onSubmit = handleSubmit(async (formValues) => {
  loading.value = true
  try {
    const { data, error, status, refresh } = await useFetch(`/api/user/update/${user.value?.id}`, {
      headers: {
          ...
          <FormField v-slot="{ componentField }" name="nickname">
            <FormItem>
              <FormLabel class="flex justify-between">
                <p>Username</p>
                <p class="text-xs text-destructive motion-preset-fade motion-preset-fade-out" v-if="isValid?.valid === false && !isValid.zod">{{ `❌ ${isValid.message}` }}</p>
                <p class="text-xs motion-preset-fade motion-preset-fade-out" v-if="isValid?.valid === true && !isValid.zod">{{ `✅ ${isValid.message}` }}</p>
              </FormLabel>
              <FormControl>
                <Input
                  type="text"
